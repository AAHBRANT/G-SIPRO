...
async function main(): Promise<void> {
  ...
  const cobre = computeArchiveAdherence({ sources: [{ text: "Terraplenagem e pavimentação asfáltica da rodovia" }], estimatedValue: 30_000_000, inferred: true }, meus);
  ...
  const falta = computeArchiveAdherence({ sources: [{ text: "Construção de ponte e terraplenagem de acesso" }], inferred: true }, meus);
  ...
  const grande = computeArchiveAdherence({ sources: [{ text: "Pavimentação asfáltica" }], estimatedValue: 300_000_000, inferred: true }, meus);
