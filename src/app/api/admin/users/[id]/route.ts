...
export async function PATCH(request: Request, route: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  ...
  return runWithRequestContext(context, async () => {
    try {
      ...
      if (current.isOwner && (!input.isOwner || input.status !== "ACTIVE")) {
      ...
      await database.$transaction(async (transaction) => {
        await transaction.user.update({ where: { id: userId }, data: { displayName: input.displayName, email: input.email, status: input.status, isMaster: input.isMaster, isOwner: input.isOwner, departmentId: input.departmentId ?? null, updatedBy: authorization.actorId } });
