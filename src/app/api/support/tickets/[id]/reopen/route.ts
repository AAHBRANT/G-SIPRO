...
export async function POST(request: Request, route: { params: Promise<{ id: string }> }) {
  ...
  return runWithRequestContext(context, async () => {
    try {
      ...
      const nextStatus = ticket.approvalRequired ? "WAITING_APPROVAL" : "TRIAGED";
      await database.$transaction(async transaction => {
        await transaction.supportTicket.update({ where: { id: ticketId }, data: { status: nextStatus, resolution: null, resolvedAt: null, resolvedById: null, assignedToId: null, executionLeaseId: null, executorId: null, executionClaimedAt: null, executionHeartbeatAt: null, executionAttempts: 0 } });
        await transaction.supportTicketUpdate.create({ data: { id: randomUUID(), ticketId, fromStatus: "RESOLVED", toStatus: nextStatus, note: input.note, createdById: authorization.actorId, actorLabel: "Proprietário" } });
