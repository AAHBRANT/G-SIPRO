  ...
  return runWithRequestContext(context, async () => {
    try {
      ...
      await database.$transaction(async transaction => {
        await transaction.supportTicket.update({ where: { id: ticketId }, data: { status: nextStatus, approvalRequired: false, approvalReason: null, resolution: null, resolvedAt: null, resolvedById: null, assignedToId: null, executionLeaseId: null, executorId: null, executionClaimedAt: null, executionHeartbeatAt: null, executionAttempts: 0, resolutionAttempts: 0, validationQuestions: Prisma.JsonNull, validationRequestedAt: null, externalBlocker: Prisma.JsonNull, ownerActionRequiredAt: null, escalatedAt: null } });
        await transaction.supportTicketUpdate.create({ data: { id: randomUUID(), ticketId, fromStatus: "RESOLVED", toStatus: nextStatus, note: `Reabertura aceita. A GUULY iniciará automaticamente uma nova sequência de até três tentativas. Motivo: ${input.note}`, createdById: authorization.actorId, actorLabel: "Proprietário" } });
