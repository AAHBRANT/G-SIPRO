...
export async function PATCH(request: Request, route: { params: Promise<{ id: string }> }) {
  ...
  return runWithRequestContext(context, async () => { try {
    const authorization = await requirePermission("analyses.reassign"); const id = z.uuid().parse((await route.params).id);
    const data = await new AnalysisService(new PrismaAnalysisRepository()).reassign(id, await request.json(), authorization.actorId, context.correlationId);
