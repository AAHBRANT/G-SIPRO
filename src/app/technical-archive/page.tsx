...
export default async function TechnicalArchivePage() {
  ...
  const [documents, users, useCases] = await Promise.all([
    database.managedDocument.findMany({
      where: { type: "ATESTADO", status: { not: "DELETED" } }, include: { owner: true, versions: { orderBy: { version: "desc" }, include: { technicalEvidenceRecords: { include: { experience: { include: { services: { include: { quantities: true } } } } } }, aiExtractionExecutions: { orderBy: { startedAt: "desc" }, take: 1 } } } }, orderBy: { createdAt: "desc" },
    }),
