...
export function ArchiveWorkspace({ items, users, canCreate, canDelete, extractionDefinitionId }: { items: ArchiveItem[]; users: { id: string; name: string }[]; canCreate: boolean; canDelete: boolean; extractionDefinitionId?: string }) {
  ...
  return <>
    ...
    <section className="mt-6 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-[0_3px_14px_rgba(15,23,42,0.05)]"><div className="flex items-center justify-between border-b border-slate-200 px-6 py-5"><div><h2 className="text-base font-black uppercase tracking-wide">Relação de acervos</h2><p className="mt-1 text-xs text-slate-500">{filtered.length} de {items.length} documentos</p></div>{message && <p className="text-xs font-semibold text-emerald-700" role="status">{message}</p>}</div><div className="overflow-x-auto"><table className="w-full min-w-[780px] text-left text-sm"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-wide text-slate-500"><tr><th className="px-5 py-3">Documento</th><th className="px-4 py-3">Contratante / emissor</th><th className="px-4 py-3">Objeto</th><th className="px-4 py-3">Serviços</th><th className="px-4 py-3">Situação</th><th className="px-5 py-3 text-center">Ações</th></tr></thead><tbody className="divide-y divide-slate-100">{filtered.map(item => { const status = extractionPresentation(item); return <tr className="cursor-pointer transition hover:bg-blue-50/50" key={item.id} onClick={() => { setSelected(item); setInsideQuery(""); }}><td className="px-5 py-4"><p className="font-bold text-slate-900">{item.title}</p><p className="mt-1 text-xs text-slate-500">{item.number ? `Nº ${item.number} · ` : ""}v{item.version} · {item.createdAt}</p></td><td className="px-4 py-4 text-slate-600">{item.contractor ?? item.issuer ?? "A reconhecer"}</td><td className="max-w-xs px-4 py-4 text-slate-600"><p className="line-clamp-2">{item.subject ?? "A reconhecer"}</p></td><td className="px-4 py-4"><span className="rounded-full bg-blue-50 px-2.5 py-1 text-xs font-bold text-brand">{item.services.length}</span></td><td className="px-4 py-4"><span className={`rounded-full px-2.5 py-1 text-xs font-bold ${status.classes}`}>{status.label}</span></td><td className="px-5 py-4 text-center"><button className="rounded-lg px-3 py-2 text-xs font-bold text-brand hover:bg-blue-50" type="button">Visualizar</button>{canDelete && <button className="rounded-lg px-3 py-2 text-xs font-bold text-rose-600 hover:bg-rose-50" onClick={(event) => { event.stopPropagation(); setDeleteTarget(item); setDeleteReason(""); setDeleteMessage(""); }} type="button">Excluir</button>}</td></tr>; })}{!filtered.length && <tr><td className="px-5 py-12 text-center text-slate-400" colSpan={6}>Nenhum acervo encontrado para esta pesquisa.</td></tr>}</tbody></table></div></section>
