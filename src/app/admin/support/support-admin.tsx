    ...
    {selectedTicket && <div className="fixed inset-0 z-50 flex justify-end" role="dialog" aria-modal="true" aria-labelledby="support-ticket-title">
      ...
      <aside className="relative h-full w-full max-w-3xl overflow-y-auto bg-[#f6f6f4] shadow-2xl">
        ...
        {selectedTicket.status === "WAITING_APPROVAL" && <section className="mt-4 rounded-xl border-2 border-amber-300 bg-amber-50 p-4">
          ...
          <p className="mt-2 text-xs leading-5 text-amber-900">Registre uma justificativa abaixo. Ao aprovar, o chamado entra automaticamente na fila da GUULY; ao rejeitar, ele é encerrado sem alteração no software.</p>
        </section>}
        ...
        {selectedTicket.status === "OWNER_ACTION_REQUIRED" && externalBlocker && <section className="mt-4 rounded-xl border-2 border-amber-300 bg-amber-50 p-4 text-sm text-amber-950">
          ...
          <p className="mt-3 text-xs font-semibold">Execute somente a ação indicada, descreva abaixo o que foi feito e confirme. A GUULY retomará o chamado automaticamente.</p>
        </section>}
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          ...
          {selectedTicket.status === "OWNER_ACTION_REQUIRED" && canApprove && <button className="rounded-lg bg-amber-600 px-4 py-2 text-xs font-bold text-white disabled:opacity-50" disabled={busy === selectedTicket.id || note.trim().length < 3} onClick={() => send(selectedTicket.id, "escalation", { note })}>Confirmar ação e devolver à GUULY</button>}
