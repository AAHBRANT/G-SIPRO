...
export default async function Home({ searchParams }: { searchParams: Promise<{ year?: string }> }) {
  ...
  return <div className="mx-auto w-full max-w-[1500px] px-4 py-5 sm:px-6 lg:px-8 lg:py-7">
    ...
    <section className="mt-6 grid gap-6 xl:grid-cols-[1fr_1.25fr]">
      ...
      <Panel action={<div className="flex items-center gap-3"><Link className="inline-flex items-center gap-1 text-xs font-bold text-brand hover:underline" href="/tenders">Ver editais <GsIcon className="h-3 w-3" name="arrow"/></Link><Link className="inline-flex items-center gap-1 text-xs font-bold text-brand hover:underline" href="/calendar">Ver calendário completo <GsIcon className="h-3 w-3" name="arrow"/></Link></div>} subtitle="Vencimentos confirmados para os próximos 30 dias" title="Próximos prazos" variant="executive">
