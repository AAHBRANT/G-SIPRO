...
export function SupportChat({ messages, currentActorId, ticketId }: { ticketId: string; messages: SupportChatMessage[]; currentActorId: string }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!response.ok) throw new Error(result?.error?.message ?? "Não foi possível enviar a solicitação à GUULY.");
      setMessage("");
      setFeedback("Solicitação recebida pela GUULY do G-SIPRO.");
      router.refresh();
    } catch (error) {
  ...
  return <section className="mt-5 overflow-hidden rounded-xl border border-slate-200 bg-slate-50">
    <header className="border-b border-slate-200 bg-white px-4 py-3"><p className="text-xs font-black uppercase tracking-wider text-slate-700">Assistente de suporte</p><p className="mt-1 text-xs text-slate-500">Diga em linguagem simples o que a GUULY deve fazer. Aprovações e ações protegidas continuam respeitando as regras do sistema.</p></header>
    ...
    <form className="border-t border-slate-200 bg-white p-3" onSubmit={submit}>
      <label className="sr-only" htmlFor={`support-assistant-${ticketId}`}>Mensagem para a GUULY do G-SIPRO</label>
      <div className="flex items-end gap-2 rounded-xl border border-slate-200 bg-slate-50 p-2 focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-100">
        <textarea
          ...
          placeholder="Ex.: GUULY, verifique este chamado e faça a correção necessária."
          required
          value={message}
        />
        <button className="shrink-0 rounded-lg bg-brand px-4 py-3 text-xs font-bold text-white disabled:opacity-50" disabled={busy || message.trim().length < 3} type="submit">{busy ? "Enviando…" : "Enviar para a GUULY"}</button>
      </div>
      {feedback && <p className="mt-2 text-xs font-semibold text-slate-600" role="status">{feedback}</p>}
      <p className="mt-2 text-[10px] leading-4 text-slate-500">A GUULY registra o pedido, executa pela fila segura e publica aqui o andamento, os testes e o resultado. Você só será chamado quando houver uma aprovação ou ação externa exclusiva.</p>
    </form>
  </section>;
}
