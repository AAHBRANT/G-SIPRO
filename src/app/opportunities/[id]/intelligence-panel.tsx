...
function ContextForm({ bases, busy, buttonLabel, defaultBaseId, defaults, onSubmit, showDates = false }: { bases?: readonly OperationalBaseOption[]; busy: boolean; buttonLabel: string; defaultBaseId?: string; defaults: AnalysisContextDefaults; onSubmit: (event: FormEvent<HTMLFormElement>) => void; showDates?: boolean }) {
  ...
  return <form className="min-w-0 rounded-xl border border-slate-200 bg-slate-50 p-5" onSubmit={handleSubmit}><h3 className="font-black text-slate-900">{bases ? "Origem e destino" : "Local da obra"}</h3><p className="mt-1 break-words text-xs text-slate-500">{defaults.sources.length > 0 ? `Sugestão encontrada nos documentos desta oportunidade: ${defaults.sources.join(", ")}. Confirme o local antes da consulta.` : "Pesquise pela cidade ou pelo endereço informado no edital."}</p><div className="mt-4 grid min-w-0 gap-3">{bases && <label className="grid min-w-0 gap-1 text-xs font-bold text-slate-700">Endereço de partida<select className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 font-normal text-slate-800" defaultValue={defaultBaseId ?? analysisBaseId(bases)} disabled={bases.length === 0} name="routeBaseId" required><option disabled value="">{bases.length === 0 ? "Cadastre uma base operacional no Administrador" : "Selecione a Matriz ou Filial"}</option>{bases.map((base) => <option className="whitespace-normal break-words" key={base.id} value={base.id}>{base.code} · {base.name} — {base.locality}</option>)}</select></label>}<label className="grid min-w-0 gap-1 text-xs font-bold text-slate-700">Cidade ou endereço da obra<div className="flex min-w-0 flex-col gap-2"><input className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 font-normal" maxLength={500} name="locationLabel" onChange={(event) => { setLocationLabel(event.target.value); setLatitude(""); setLongitude(""); setLocationMessage(""); }} placeholder="Ex.: Gravataí/RS ou endereço completo do edital" required value={locationLabel}/><button className="shrink-0 rounded-lg border border-brand bg-white px-4 py-2 text-xs font-bold text-brand disabled:opacity-50" disabled={searching} onClick={locateAddress} type="button">{searching ? "Localizando…" : "Localizar"}</button></div></label>{locationMessage && <p aria-live="polite" className="break-words text-xs font-semibold text-slate-600" role="status">{locationMessage}</p>}<details className="min-w-0 rounded-lg border border-slate-200 bg-white"><summary className="cursor-pointer break-words px-3 py-2 text-xs font-bold text-slate-700">Localização avançada · latitude e longitude</summary><div className="grid min-w-0 gap-3 border-t border-slate-100 p-3"><label className="grid min-w-0 gap-1 text-xs font-bold text-slate-700">Latitude<input className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 font-normal" max="90" min="-90" name="latitude" onChange={(event) => setLatitude(event.target.value)} placeholder="-29.9448" step="any" type="number" value={latitude}/></label><label className="grid min-w-0 gap-1 text-xs font-bold text-slate-700">Longitude<input className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 font-normal" max="180" min="-180" name="longitude" onChange={(event) => setLongitude(event.target.value)} placeholder="-50.9919" step="any" type="number" value={longitude}/></label><p className="break-words text-[10px] leading-4 text-slate-500">Use somente quando a busca por cidade/endereço não oferecer precisão suficiente.</p></div></details>{showDates && <><label className="grid min-w-0 gap-1 text-xs font-bold text-slate-700">Início previsto<input className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 font-normal" defaultValue={defaults.workStart} name="workStart" required type="date"/></label><label className="grid min-w-0 gap-1 text-xs font-bold text-slate-700">Fim previsto<input className="w-full min-w-0 rounded-lg border border-slate-200 bg-white px-3 py-2 font-normal" defaultValue={defaults.workEnd} name="workEnd" required type="date"/></label></>}</div><button className="mt-4 w-full rounded-lg bg-brand px-4 py-2.5 text-xs font-bold text-white disabled:opacity-50" disabled={busy || searching || Boolean(bases && bases.length === 0)}>{busy ? "Processando…" : buttonLabel}</button></form>;
}
