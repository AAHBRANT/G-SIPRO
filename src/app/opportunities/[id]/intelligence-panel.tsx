    ...
    <section aria-labelledby="intelligence-title" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.06)]">
          ...
          <div className="grid gap-4 p-5 lg:grid-cols-3 sm:p-7">
            ...
            {(perspective === "ALL" || perspective === "STUDIES") && (
              <>
                ...
                <FeatureCard description={analysis.route ? `${analysis.route.alternatives.length} alternativa(s) de mobilização calculada(s).` : "Compare distância, tempo e rota entre as bases e a obra."} icon="target" label="Logística e distância" onOpen={() => { setMessage(""); setDrawer("LOGISTICS"); }} status={analysis.route ? "Rotas disponíveis" : "Aguardando estudo"}/>
              </>
            )}
      ...
      {drawer && (
        <IntelligenceDrawer title={drawerTitle(drawer)} onClose={() => setDrawer(null)} closeButtonRef={closeButtonRef}>
          {drawer === "CLIMATE" && <ClimateDetail analysis={analysis} busy={busyStage === "CLIMATE"} canCalculate={canCalculate} contextDefaults={contextDefaults} message={message} onSubmit={(event) => runContextualStage(event, "CLIMATE")}/>}
          {drawer === "LOGISTICS" && <LogisticsDetail analysis={analysis} bases={operationalBases} busy={busyStage === "LOGISTICS"} canCalculate={canCalculate} contextDefaults={contextDefaults} message={message} onSubmit={(event) => runContextualStage(event, "LOGISTICS")}/>}
...
function LogisticsDetail({ analysis, bases, canCalculate, busy, contextDefaults, message, onSubmit }: { analysis: IntelligenceAnalysisView | null; bases: readonly OperationalBaseOption[]; canCalculate: boolean; busy: boolean; contextDefaults: AnalysisContextDefaults; message: string; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  const routeDefaults: AnalysisContextDefaults = route
    ...
    : contextDefaults;
  return <div className="space-y-6">{route ? <><section className="overflow-hidden rounded-xl border border-slate-200 bg-slate-100"><div className="relative grid min-h-80 place-items-center">{mapUrl ? <Image alt={`Rota da base ${selected?.baseName} para ${route.destinationLabel}`} className="object-cover" fill sizes="(max-width: 768px) 100vw, 768px" src={mapUrl} unoptimized/> : <div className="px-6 text-center"><span className="mx-auto grid h-12 w-12 place-items-center rounded-full bg-white text-brand shadow"><GsIcon name="target"/></span><h3 className="mt-4 font-black text-slate-900">{route.destinationLabel}</h3><p className="mt-2 max-w-md text-xs leading-5 text-slate-500">Nenhuma rota válida foi retornada para exibição no Azure Maps.</p></div>}</div></section><section className="rounded-xl border border-slate-200"><header className="border-b border-slate-100 px-4 py-3"><h3 className="font-black text-slate-900">Rota calculada</h3><p className="mt-1 text-xs text-slate-500"><span className="font-bold text-slate-700">Chegada:</span> {route.destinationLabel}</p></header><div className="divide-y divide-slate-100">{route.alternatives.map((item) => <article className="grid gap-3 px-4 py-4 sm:grid-cols-[1fr_auto_auto_auto_auto] sm:items-center" key={item.baseId}><div><p className="text-[9px] font-bold uppercase tracking-wide text-slate-400">Partida</p><p className="font-bold text-slate-900">{item.baseCode} · {item.baseName}</p><p className="text-xs text-slate-500">{item.baseLocality}</p></div><MiniMetric label="Distância" value={item.distanceKm === undefined ? "Sem rota" : `${item.distanceKm.toLocaleString("pt-BR")} km`}/><MiniMetric label="Duração" value={item.durationHours === undefined ? "—" : `${item.durationHours.toLocaleString("pt-BR")} h`}/><MiniMetric label="Pedágios" value={formatRouteTolls(item.tolls)}/>{item.condition === "ROUTE_EXISTS" ? <button aria-label={`Ver rota da base ${item.baseName} no mapa acima`} className="text-xs font-bold text-brand hover:underline" onClick={() => setSelectedBaseId(item.baseId)} type="button">Visualizar</button> : <span className="text-xs text-slate-400">Sem rota</span>}</article>)}</div></section></> : <EmptyDetail text="Ainda não há estudo logístico. Selecione uma base operacional e informe o local da obra."/>}{message && <p aria-live="polite" className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-xs font-semibold leading-5 text-blue-900" role="status">{message}</p>}{canCalculate && <ContextForm bases={bases} busy={busy} buttonLabel="Calcular rota" defaultBaseId={route?.selectedBaseId} defaults={routeDefaults} onSubmit={onSubmit}/>}</div>;
}
