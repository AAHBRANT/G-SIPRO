    ...
    <section aria-labelledby="intelligence-title" className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.06)]">
      ...
      {drawer && (
        <IntelligenceDrawer title={drawerTitle(drawer)} onClose={() => setDrawer(null)} closeButtonRef={closeButtonRef}>
          ...
          {drawer === "LOGISTICS" && <LogisticsDetail analysis={analysis} bases={operationalBases} busy={busyStage === "LOGISTICS"} canCalculate={canCalculate} contextDefaults={contextDefaults} onSubmit={(event) => runContextualStage(event, "LOGISTICS")}/>}
...
function LogisticsDetail({ analysis, bases, canCalculate, busy, contextDefaults, onSubmit }: { analysis: IntelligenceAnalysisView | null; bases: readonly OperationalBaseOption[]; canCalculate: boolean; busy: boolean; contextDefaults: AnalysisContextDefaults; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
