...
export const supportDiagnosisSchema = z.object({
  ...
  requiredActor: z.enum(["AI", "MASTER", "OWNER", "REQUESTER"]),
  ownerActionCategory: z.enum(["MICROSOFT_365", "TEAMS", "AZURE", "IDENTITY_ACCESS", "SECURITY", "EXTERNAL_SERVICE", "OTHER"]).nullable(),
  requiredAction: z.string().trim().min(1).max(4_000).nullable(),
  securityGuidance: z.string().trim().min(1).max(2_000).nullable(),
