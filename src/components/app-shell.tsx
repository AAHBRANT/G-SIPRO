...
const icons = {
  ...
  scouting: <Icon><circle cx="11" cy="11" r="7"/><path d="m20 20-4.3-4.3"/></Icon>,
};
...
function Navigation({ close, isMaster = false, isOwner = false, pendingApprovals = 0, pendingScouted = 0, unreadNotifications = 0, permissions }: { close?: () => void; isMaster?: boolean; isOwner?: boolean; pendingApprovals?: number; pendingScouted?: number; unreadNotifications?: number; permissions: ReadonlyArray<string> }) {
  const pathname = usePathname();
  const granted = new Set(permissions);
  const mainItems = navigation
    .filter((item) => isMaster || !item.permission || granted.has(item.permission))
    // O aviso de licitações rastreadas aponta para Oportunidades, onde vive a
    // fila de triagem — e não para o Buscador, que é tela de configuração.
    .map((item) => (item.href === "/opportunities" && pendingScouted > 0 ? { ...item, badge: pendingScouted } : item));
...
export function AppShell({ children, userLabel, isMaster = false, isOwner = false, pendingApprovals = 0, pendingScouted = 0, unreadNotifications = 0, permissions = [], signOutAction }: { children: ReactNode; userLabel: string; isMaster?: boolean; isOwner?: boolean; pendingApprovals?: number; pendingScouted?: number; unreadNotifications?: number; permissions?: ReadonlyArray<string>; signOutAction: () => Promise<void> }) {
  ...
  return <div className="min-h-screen bg-background">
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col border-r border-slate-800 bg-[#1d1e21] md:flex">
      ...
      <Navigation isMaster={isMaster} isOwner={isOwner} pendingApprovals={pendingApprovals} pendingScouted={pendingScouted} permissions={permissions} unreadNotifications={unreadNotifications}/>
    ...
    {/* O botão flutuante de suporte não aparece no Buscador: a tela já tem
        ação própria em todo lugar que precisa de atenção (reler edital,
        aprovar/descartar), e o botão sobrepunha a fila sem servir a essa
        funcionalidade específica. */}
    {!pathname.startsWith("/scouting") && !pathname.startsWith("/opportunities/scouted") && <Link aria-label="Abrir suporte" className="fixed bottom-5 right-5 z-30 inline-flex items-center gap-2 rounded-full bg-brand px-4 py-3 text-sm font-bold text-white shadow-lg shadow-red-950/15 transition hover:-translate-y-0.5 hover:bg-[var(--brand-strong)]" href={`/support?from=${encodeURIComponent(pathname)}`}>{icons.support}<span className="hidden sm:inline">Suporte</span></Link>}
  </div>;
}
